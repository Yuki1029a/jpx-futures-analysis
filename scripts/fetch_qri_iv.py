...
# Numeric helpers
# _num() はマッチ前にカンマを除去するため、桁数制限のない単純形にすること。
# 旧版 r"-?\d{1,3}(?:,\d{3})*..." はカンマ除去後の4桁以上を先頭3桁に切り詰める
# バグがあった（"1,006"→100）。2026-07-12修正。それ以前の parquet の
# oi/volume/settle/last 等は1,000以上の値が破損している。
